...
|---|---|---|
| 1 | 15 min | Retomada: o que já sabemos fazer, das descritivas à relação entre variáveis |
| 2 | 55 min | Exposição dialogada: estrutura do relatório; escrita científica; reprodutibilidade e notebook; slide único |
| 3 | 15 min | Intervalo |
| 4 | **40 min** | **Bloco de estatística: a tabela de correspondência e a leitura crítica** (seção 4.E) |
| 5 | 55 min | Oficina de redação: aplicar a estrutura ao próprio projeto com feedback |
| 6 | 50 min | Apresentações sintéticas (3 a 5 min por estudante) |
| 7 | 10 min | Síntese e encerramento da Avaliação 3 |
...
### Bloco 2 – A estrutura do relatório (20 min)
...
### Bloco 2 – A escrita científica (15 min)
...
### Bloco 2 – O notebook reprodutível (10 min)
...
### 4.E Bloco de estatística: a tabela de correspondência (40 min)

**Slides:** bloco "Estatística", quatro telas, depois do slide único.

Este é o bloco de fechamento, e ele tem uma função prática imediata: os estudantes vão escrever a
seção de resultados na oficina que vem logo em seguida.

A **tabela de correspondência** ocupa a maior parte do tempo. Ela tem três colunas, pergunta,
estatística e formato da frase, e cobre os treze blocos do semestre. Percorrer linha a linha,
pedindo que cada estudante identifique em voz de pergunta qual linha corresponde ao próprio
projeto. Quem não achar a linha tem um problema de delineamento, não de redação, e isso precisa
ser resolvido antes da oficina.

A **leitura crítica** traz três frases com número certo e leitura incompleta. São os três erros
recorrentes: relatar média sem escala nem dispersão, usar verbo causal sobre dado observacional, e
tratar "não significativo" como prova de igualdade. Vale pedir que a turma corrija cada uma antes
de mostrar a correção.

O **mapa das fichas** fecha o bloco e serve de guia de estudo para a prova final: treze telas para
reler, mais a execução das seções "Estatística do encontro" nos notebooks, o que dá cerca de duas
horas de revisão.

**Gancho com a Avaliação 3:** o critério de correção do relatório inclui a frase de leitura de cada
resultado, no formato da terceira coluna da tabela. Anunciar isso aqui, antes da oficina.
