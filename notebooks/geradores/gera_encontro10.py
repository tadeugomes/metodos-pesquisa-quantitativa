...
fig, ax = plt.subplots(figsize=(10, 5))
ax.boxplot([g.values for _, g in anos], tick_labels=[a for a, _ in anos])
...
C.append({"tipo": "md", "texto": """\
## Estatística do encontro: classes e escore z

**As fórmulas.** A amplitude de cada classe é `h = (máximo − mínimo) / número de classes`. O
histograma é essa tabela desenhada. O escore z é `z = (x − x̄) / s`, e diz quantos desvios padrão
um valor está da média.

**A régua.** Quando a distribuição é aproximadamente simétrica, cerca de 68% dos casos ficam entre
z = −1 e +1, 95% entre −2 e +2 e 99,7% entre −3 e +3. É o que transforma "foi um mês atípico" em
número."""})

C.append({"tipo": "code", "texto": """\
# a tabela de frequência com classes, que o histograma desenha
classes = pd.cut(mensal["inadimplencia_pj"], bins=6)
tabela_classes = classes.value_counts().sort_index().to_frame("meses")
tabela_classes["%"] = (tabela_classes["meses"] / len(mensal) * 100).round(1)
tabela_classes"""})

C.append({"tipo": "code", "texto": """\
# o mesmo dado com três números de classes: a escolha muda o desenho
fig, eixos = plt.subplots(1, 3, figsize=(13, 3.4))
for eixo, k in zip(eixos, [4, 10, 30]):
    eixo.hist(mensal["inadimplencia_pj"].dropna(), bins=k)
    eixo.set_title(f"bins = {k}")
plt.tight_layout()
plt.show()"""})

C.append({"tipo": "code", "texto": """\
# o escore z de cada mês, e os meses atípicos
v = mensal["inadimplencia_pj"]
z = (v - v.mean()) / v.std()

print(f"média = {v.mean():.2f} | desvio padrão = {v.std():.2f}")
print("\\nMeses a mais de 2 desvios padrão da média:")
print(z[z.abs() > 2].round(2))"""})

C.append({"tipo": "md", "texto": """\
**Cuidado com a régua.** A regra 68-95-99,7 pressupõe distribuição aproximadamente simétrica. Em
variáveis com cauda longa, como faturamento, use percentis: "está acima do percentil 95".

**Escreva a sua leitura, no formato de legenda de figura.**

*A inadimplência mensal concentra-se entre ______% e ______%, com distribuição ______________
(simétrica ou assimétrica?). O mês de ______________, com ______%, está a ______ desvios padrão da
média do período. Figura construída com ______ classes; fonte: Banco Central, série ______.*"""})
